...
// Define the expected type for salary data
...
const DashboardView = ({ insights }: { insights: IndustryInsight }) => {
    // Type-cast salaryRanges as unknown first, then to SalaryData[]
    const salaryData = ((insights.salaryRanges as unknown) as SalaryData[]).map((range) => ({
        name: range.role,
        min: range.min / 1000,
        max: range.max / 1000,
